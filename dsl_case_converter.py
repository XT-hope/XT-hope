...
def parse_set_step(line: str) -> DslSetStep:
    ...
    cut_pos = _find_first_keyword_pos(body, [" then check ", " keep_dynamic ", " keepdynamic ", " wait "])
    expr = body
...
def parse_check_def(line: str) -> DslCheckDef:
    ...
    t_m = re.search(r"\btimeoutOfCheck\s+(?P<t>\S+)\b", body, flags=re.IGNORECASE)
    if t_m:
        timeout_s = parse_time_seconds(t_m.group("t"))

    d_m = re.search(r"\bcheckInTime\s+(?P<t>\S+)\b", body, flags=re.IGNORECASE)
    if d_m:
    ...
    cut_pos = _find_first_keyword_pos(body, [" timeoutofcheck ", " checkintime ", " wait "])
    expr = body
    if cut_pos is not None:
        expr = body[:cut_pos].strip()
    ...
    # expr: sig::CAN ...::Signal ==3 | in {2,3} | in 2..5
    sig_m = re.match(
...
def _check_to_actions(c: DslCheckDef) -> Dict[str, Any]:
    base: Dict[str, Any]
...
def convert_case_to_python_module(dsl: DslCase) -> str:
    ...
    def emit_set_step(step: DslSetStep) -> None:
        ...
        urltests.append({"description": step.sid, "steps": steps})
        ...
        if step.then_checks:
            ...
            for cid in step.then_checks:
                ...
                if not cdef:
                    missing.append(cid_u)
                    continue
                then_steps.append(_check_to_actions(cdef))
                consumed_inline_checks.add(cid_u)

            desc = f"{step.sid} then CHECK {','.join(step.then_checks)}"
            if missing:
                desc += f" (missing: {','.join(missing)})"
            urltests.append({"description": desc, "steps": then_steps})
    ...
    for cid in dsl.checks_in_order:
        if cid in consumed_inline_checks:
            continue
        cdef = dsl.checks[cid]
        urltests.append({"description": cid, "steps": [_check_to_actions(cdef)]})
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--output-dir", type=str, default=None, help="输出目录（默认与输入文件同目录）")
