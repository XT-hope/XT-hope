...
class MainWindow(QMainWindow):
    ...
    def update_project_tree(self, *, restore_selection: bool = True) -> None:
        ...
        if saved_current_data:
            current_restored = self._restore_tree_item_by_data(saved_current_data)
        ...
        for item_data in items_data:
            ...
            if item_data.get("type") == "file":
                ...
                if file_path.endswith('.dsl'):
                    ...
                    delete_tasks.append({"type": "file", "case_name": cn, "directory": d})
            elif item_data and item_data.get("type") == "directory":
                ...
                delete_tasks.append({"type": "directory", "directory": d})
                ...
                if item_type == "automation_file":
                    ...
                    if fp.exists():
                        with open(fp, 'r', encoding='utf-8') as f:
                            content = f.read()
                        batch_entries.append({
                            "kind": "file", "file_path": item_data.get("path", ""),
                            "case_type": case_type, "content": content
                        })
                        delete_tasks.append({
                            "type": "automation_file", "path": item_data.get("path", ""),
                            "case_type": case_type, "base": base, "fp": fp
                        })
                elif item_type == "automation_directory":
                    ...
                    if dp.exists():
                        ...
                        batch_entries.append({
                            "kind": "directory", "dir_path": item_data.get("path", ""),
                            "case_type": case_type, "files": dir_files
                        })
                        delete_tasks.append({
                            "type": "automation_directory", "path": item_data.get("path", ""),
                            "case_type": case_type, "base": base, "dp": dp
                        })
            except Exception:
                pass
        ...
        for task in delete_tasks:
            try:
                if task["type"] == "automation_file":
                    ...
                    success_count += 1
                elif task["type"] == "automation_directory":
                    dp = task["dp"]
                    if dp.exists():
