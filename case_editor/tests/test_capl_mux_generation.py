...
class CaplMuxGenerationTest(unittest.TestCase):
    ...
    def test_core_burst_vars_declared_without_msg_send_type(self) -> None:
        ...
        self.assertIn("void poll_emit_Media_0x32B(long tNow)", content)
        self.assertIn("void poll_prepare_Media_0x32B()", content)
        self.assertIn("poll_emit_Media_0x32B(tNow);", content)
        self.assertIn("poll_prepare_Media_0x32B();", content)
        self.assertIn("  emit_Media_0x32B();", content)
        self.assertIn("  advance_due_Media_0x32B(tNow);", content)
        self.assertIn("  due_Media_0x32B = _dueBase + _ct * 100;", content)
