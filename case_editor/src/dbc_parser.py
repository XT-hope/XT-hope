...
class DBCParser:
    ...
    def get_signal_info(self, signal_path: str, signal_type: str = "sig") -> Optional[Dict[str, Any]]:
        """
        获取信号的值定义信息（枚举值或范围）

        Args:
            signal_path: 信号路径（不含类型前缀，格式: "CAN 1::Message::Signal"）
            signal_type: 信号类型 ("sig" 或 "env")

        Returns:
            Dict with keys:
            - "choices": OrderedDict {value: description} 或 None
            - "minimum": float 或 None
            - "maximum": float 或 None
            或 None if signal not found
        """
        if signal_type not in ("sig", "env"):
            return None
        ...
        # 解析信号路径
        # 格式: "CAN X::Message::Signal" 或完整路径 "sig::CAN X::Message::Signal"
        # 先去除可能的前缀
        if signal_path.startswith("sig::"):
            signal_path = signal_path[5:]
        elif signal_path.startswith("env::"):
            signal_path = signal_path[5:]
        ...
        try:
            can_part = parts[0]  # "CAN 1"
            msg_name = parts[1]
            sig_name = parts[2]

            # 提取通道号
            channel_str = can_part.replace("CAN ", "").strip()
            channel = int(channel_str) - 1  # 转换为0-based索引

        except (ValueError, IndexError):
            return None

        # 查找对应的DBC文件
        target_dbc_path = None
        for dbc_path, mapping_info in self.can_channel_mapping.items():
            dbc_channel = mapping_info.get("channel") if isinstance(mapping_info, dict) else mapping_info
            if dbc_channel == channel:
                target_dbc_path = dbc_path
                break

        if target_dbc_path is None or target_dbc_path not in self.dbc_files:
            return None

        db = self.dbc_files[target_dbc_path]
        ...
        # 返回信号信息
        return {
            "choices": dict(sig.choices) if sig.choices else None,
            "minimum": sig.minimum,
            "maximum": sig.maximum,
        }
