"""
CAPL 生成器（启动相位按 CAN ID 错开试验版）

本文件由 capl_generation2.py 复制而来，仅改相位序号规则；请不要回写
capl_generation.py / capl_generation2.py。

周期报文 arm_start 的序号按 CAN ID 升序分配（ID 越小序号越小），与配置列表顺序无关。
无 DBC ID 的报文排在有 ID 的后面，并保持原相对顺序。同 ID 时按原遍历顺序稳定编号。
不按周期长短排序。
...
- 周期 timer：emit → arm → fill。arm() 仍 setTimer(周期)，保证热路径 delay>0、链不断。
  on start / MsgOn 走 arm_start()：setTimer(1 + (index % cycle))，把各报文首次到期错开。
  index 按 CAN ID 升序全局编号（ID 越小越优先），不按周期、不按配置列表顺序。
  首次 delay 始终 >= 1 ms，不用 timeNow()。
...
def _periodic_phase_sort_key(frame_id: Optional[int], orig_order: int) -> Tuple:
    """CAN ID 升序；无 ID 排在有 ID 之后；同 ID / 都无 ID 时保持 orig_order。"""
    if frame_id is None:
        return (1, orig_order)
    return (0, int(frame_id), orig_order)
...
def _periodic_phase_indices(
    ...
    """按 CAN ID 升序给周期报文分配启动相位 index。"""
    frame_ids = frame_ids or {}
    ranked: List[Tuple[Tuple, str]] = []
    orig_order = 0
    for _, model in messages:
        if not _needs_periodic_scheduler(model):
            continue
        key = _periodic_phase_sort_key(frame_ids.get(model.name), orig_order)
        ranked.append((key, model.name))
        orig_order += 1
    ranked.sort(key=lambda item: item[0])
    return {name: start + index for index, (_key, name) in enumerate(ranked)}
...
def _build_can_file(
    ...
    phases: 报文名 -> 启动相位序号。缺省则按本文件 CAN ID 升序从 phase_index 起编。
    """
    namespace = dbc_name
    frame_ids = frame_ids or {}
    if phases is None:
        phases = _periodic_phase_indices(messages, phase_index, frame_ids)
    out: List[str] = []
    out.append("/*@!Encoding:65001*/")
    out.append(f"/* 自动生成：{dbc_name} 网络中 {sender_node} 节点的报文模拟发送 */")
    out.append("/* capl_generation3：arm_start 序号按 CAN ID 升序，1+(index%cycle) */")
    out.append("")
    out.append("includes")
    out.append("{")
    ...
    for msg_cfg, model in messages:
        ...
        out.extend(
            _build_arm_start_function(
                namespace, name, parsed, model, phases.get(name, 0)
            )
        )
        out.extend(
...
def _build_arm_start_function(
    ...
    """首次预约：delay = 1 + (index % cycle)，始终 >= 1 ms，不用 timeNow。"""
    if not _needs_periodic_scheduler(model):
        return []
    lines = [
        f"void arm_start_{message_name}()",
        "{",
        "  long _ct;",
        "  long _ph;",
    ]
    ...
    lines.extend([
        f"  _ph = 1 + ({int(phase_index)} % _ct);",
        f"  setTimer(tmr_{message_name}, _ph);",
        "}",
        "",
    ])
    return lines
...
def generate_capl(config: Dict[str, Any], project_path) -> List[str]:
    ...
    generated: List[str] = []
    jobs: List[Tuple[str, str, int, List[Tuple[Dict[str, Any], MessageModel]], Dict[str, int]]] = []
    rank_items: List[Tuple[Tuple, str, str, str]] = []
    orig_order = 0
        ...
        for sender in dbc_config.get("senders", []):
            ...
            for msg_cfg in sender.get("messages", []):
                ...
                if _needs_periodic_scheduler(model):
                    key = _periodic_phase_sort_key(frame_ids.get(model.name), orig_order)
                    rank_items.append((key, dbc_name, sender_node, model.name))
                    orig_order += 1

            if not messages:
                continue
            jobs.append((dbc_name, sender_node, channel, messages, frame_ids))

    rank_items.sort(key=lambda item: item[0])
    global_phases: Dict[Tuple[str, str, str], int] = {}
    for index, (_key, dbc_name, sender_node, msg_name) in enumerate(rank_items):
        global_phases[(dbc_name, sender_node, msg_name)] = index

    for dbc_name, sender_node, channel, messages, frame_ids in jobs:
        phases = {
            model.name: global_phases[(dbc_name, sender_node, model.name)]
            for _cfg, model in messages
            if _needs_periodic_scheduler(model)
        }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="capl_generation3：启动相位按 CAN ID 升序错开"
    )
    parser.add_argument(
        "--project",
        required=True,
