...
本文件由 capl_generation.py 复制而来，仅改启动定时；请不要回写 capl_generation.py。
...
- 周期 timer：emit → arm → fill。arm() 仍 setTimer(周期)，保证热路径 delay>0、链不断。
  ...
  避免 10ms/100ms 在 t=0、100、200… 同时挤进 RT Kernel。index 按生成顺序全局递增。
...
def _periodic_phase_indices(
    ...
    """按生成顺序给周期报文分配启动相位 index（跨节点可从 start 续编）。"""
    phases: Dict[str, int] = {}
    index = start
    for _, model in messages:
        if _needs_periodic_scheduler(model):
            phases[model.name] = index
            index += 1
    return phases
...
def _build_can_file(
    ...
    phase_index: 本节点周期报文启动相位的起始序号，跨节点由 generate_capl 续编。
    """
    namespace = dbc_name
    phases = _periodic_phase_indices(messages, phase_index)
    out: List[str] = []
    out.append("/*@!Encoding:65001*/")
    out.append(f"/* 自动生成：{dbc_name} 网络中 {sender_node} 节点的报文模拟发送 */")
    out.append("/* capl_generation2：on start/MsgOn 使用 arm_start，首次 delay=1+(index%cycle) */")
    out.append("")
    out.append("includes")
    out.append("{")
...
def generate_capl(config: Dict[str, Any], project_path) -> List[str]:
    ...
    for dbc_config in config.get("dbc_configs", []):
        ...
        for sender in dbc_config.get("senders", []):
            ...
            phase_index += sum(
                1 for _, model in messages if _needs_periodic_scheduler(model)
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="capl_generation2：启动相位错开版 CAPL 生成"
    )
    parser.add_argument(
        "--project",
        required=True,
