...
class ProjectManager:
    ...
    def add_panel_file(self, file_path: str, namespace: str = "", message_name: str = "") -> bool:
        """
        添加CANoe面板文件到项目配置

        Args:
            file_path: 面板文件相对路径（相对于项目根目录）
            namespace: 面板所属的namespace
            message_name: 面板对应的message名称
        ...
        try:
            ...
            if "canoe" not in self.project_config:
                self.project_config["canoe"] = {}
            if "panel_files" not in self.project_config["canoe"]:
                self.project_config["canoe"]["panel_files"] = []
            ...
            # 检查是否已存在同名文件
            existing_files = [f for f in self.project_config["canoe"]["panel_files"] if f["name"] == file.name]
            if existing_files:
                # 更新现有文件
                idx = self.project_config["canoe"]["panel_files"].index(existing_files[0])
                self.project_config["canoe"]["panel_files"][idx] = file_info
            else:
                # 添加新文件
                self.project_config["canoe"]["panel_files"].append(file_info)
    ...
    def get_panel_files(self) -> List[Dict[str, Any]]:
        """获取项目中的面板文件列表"""
        return self.project_config.get("canoe", {}).get("panel_files", [])

    def remove_panel_file(self, file_name: str) -> bool:
        """
        从项目配置中移除面板文件
        ...
        try:
            if "canoe" in self.project_config and "panel_files" in self.project_config["canoe"]:
                self.project_config["canoe"]["panel_files"] = [
                    f for f in self.project_config["canoe"]["panel_files"]
                    if f["name"] != file_name
                ]
                return self.save_project()
            return True
    ...
    def get_panel_dir(self) -> Path:
        """获取面板文件目录路径"""
        if self.current_project_path:
            return self.current_project_path / "CANoe" / "panel_files"
        return None
    ...
    def remove_simulink_file(self, file_name: str) -> bool:
        """
        从项目配置中移除Simulink文件
        ...
        try:
            if "simulink" in self.project_config and "files" in self.project_config["simulink"]:
                self.project_config["simulink"]["files"] = [
                    f for f in self.project_config["simulink"]["files"]
                    if f["name"] != file_name
                ]
                return self.save_project()
            return True
        except Exception as e:
            print(f"移除Simulink文件失败: {e}")
            return False
