...
class DbcSignal:
	...
	send_type_value: Decimal = Decimal("0")
	send_type_choices: List[ValueTableEntry] = field(default_factory=list)
	is_multiplexer: bool = False
	multiplexer_signal_name: Optional[str] = None
...
def convert_cantools_message(message) -> DbcMessage:
	...
	return DbcMessage(
		...
		is_multiplexed=bool(message.is_multiplexed()),
	)
...
def convert_cantools_signal(signal) -> DbcSignal:
	send_type_choices = get_signal_attribute_choices(signal, "GenSigSendType") or default_signal_send_type_choices()
	send_type_value = resolve_signal_send_type_value(signal, send_type_choices)
	multiplexer_ids = sorted(signal.multiplexer_ids) if signal.multiplexer_ids else []
	return DbcSignal(
		name=signal.name,
		...
		send_type_value=send_type_value,
		send_type_choices=send_type_choices,
		is_multiplexer=bool(signal.is_multiplexer),
		multiplexer_signal_name=signal.multiplexer_signal,
...
def default_signal_send_type_choices() -> List[ValueTableEntry]:
	return [
		ValueTableEntry(Decimal(index), name)
		for index, name in enumerate(["Cycle", "OnWrite", "OnChange", "Event", "NoSigSendType"])
	]
...
def get_signal_attribute_definition(signal, name: str):
	return getattr(signal.dbc, "_attribute_definitions", {}).get(name) if signal.dbc else None


def get_signal_attribute_default_value(signal, name: str):
	definition = get_signal_attribute_definition(signal, name)
	return getattr(definition, "default_value", None) if definition is not None else None


def get_signal_attribute_choices(signal, name: str) -> List[ValueTableEntry]:
	definition = get_signal_attribute_definition(signal, name)
...
def resolve_signal_send_type_value(signal, choices: List[ValueTableEntry]) -> Decimal:
	value = get_signal_attribute_value(signal, "GenSigSendType")
	if value is None:
		value = get_signal_attribute_default_value(signal, "GenSigSendType")
	if value is None:
		value = "Cycle"
...
def message_info_member_specs(
	...
	# 所有发送类型都记录 _MsgCycleTime（值取 DBC 的 GenMsgCycleTime，读到多少记多少，未定义则为 0）
	specs.append(
		(
	...
	if message.info.send_type_name in ("CE", "IfActive", "CA"):
		specs.append(
			(
	...
	if message.info.send_type_name in ("CE", "Event", "IfActive", "CA"):
		specs.append(
			(
	...
	specs.extend(
		[
			(f"{prefix}_WrongCRCFlag", Decimal("0"), Decimal("0"), Decimal("1"), []),
			(f"{prefix}_WrongCounterFlag", Decimal("0"), Decimal("0"), Decimal("1"), []),
			(
				f"{prefix}_is_multiplexed",
				Decimal("1") if message.is_multiplexed else Decimal("0"),
				Decimal("0"),
				Decimal("1"),
				[],
			),
...
def add_message_struct_and_variable(
	...
	for signal in message.signals:
		...
		add_signal_send_type_member(struct_element, signal)
		bitcount += member_bitcount("int")

		if message.is_multiplexed:
			add_signal_is_multiplexer_member(struct_element, signal)
			bitcount += member_bitcount("int")
			if not signal.is_multiplexer and signal.multiplexer_ids:
				add_signal_multiplexer_id_member(struct_element, signal)
				bitcount += member_bitcount("int")
...
def add_signal_send_type_member(struct_element: ET.Element, signal: DbcSignal) -> None:
	send_type_max = (
		Decimal(len(signal.send_type_choices) - 1)
		if signal.send_type_choices
		else None
	)
	send_type_member = ET.SubElement(
		struct_element,
		"structMember",
		struct_member_attrs(
			name=f"{signal.name}_SigSendType",
			comment="",
			is_signed=False,
			member_type="int",
			start_value=signal.send_type_value,
			min_value=Decimal("0"),
			max_value=send_type_max,
			bitcount=32,
		),
	)
	if signal.send_type_choices:
		add_value_table(
			send_type_member,
			f"{signal.name}_SigSendTypeVt",
			signal.send_type_choices,
		)


def add_signal_is_multiplexer_member(struct_element: ET.Element, signal: DbcSignal) -> None:
	is_mux_member = ET.SubElement(
		struct_element,
		"structMember",
		struct_member_attrs(
			name=f"{signal.name}_is_multiplexer",
			comment="",
			is_signed=False,
			member_type="int",
			start_value=Decimal("1") if signal.is_multiplexer else Decimal("0"),
			min_value=Decimal("0"),
			max_value=Decimal("1"),
			bitcount=32,
		),
	...
	add_value_table(
		is_mux_member,
		f"{signal.name}_is_multiplexerVt",
		[
			ValueTableEntry(Decimal("0"), "no"),
			ValueTableEntry(Decimal("1"), "yes"),
		],
...
def add_signal_multiplexer_id_member(struct_element: ET.Element, signal: DbcSignal) -> None:
	mux_id = signal.multiplexer_ids[0] if signal.multiplexer_ids else 0
	ET.SubElement(
		struct_element,
		"structMember",
		struct_member_attrs(
			name=f"{signal.name}_multiplexer_id",
			comment="",
			is_signed=False,
			member_type="int",
			start_value=Decimal(str(mux_id)),
			min_value=Decimal("0"),
			max_value=Decimal("255"),
			bitcount=32,
		),
	)
...
def signal_member_specs(
	...
	value_member_type = "int" if should_use_int_for_physical_value(signal) else "float"
	return (
		...
		("Factor", "float", signal.factor, None, None, False),
		("Offset", "float", signal.offset, None, None, False),
	)
...
def add_value_table(
	...
	for entry in entries:
		...
		description = entry.description or "undefined"
		ET.SubElement(
			value_table,
			"valuetableentry",
			{
...
if __name__ == "__main__":
	dbc_files = [
		("control", r"D:\Test\case_editor\dbc\VCP_车载计算平台_Control(车控网)_Matrix_V1.4.1.1_20250926.dbc"),
		("chassis", r"D:\Test\case_editor\dbc\VCP_车载计算平台_Chassis(底盘网)_Matrix_V1.4.1.1_20250926.dbc"),
	]
